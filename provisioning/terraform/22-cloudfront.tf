...
# This resource implicitly depends on the 'aws_acm_certificate_validation.magic-helpdesk-integration' resource
...
resource "aws_cloudfront_distribution" "magic-helpdesk-integration" {
  enabled = true
  comment = "MAGIC Helpdesk (Integration)"
  ...
  aliases = [
    aws_s3_bucket.magic-helpdesk-integration.bucket,
  ]
  ...
  origin {
    domain_name = aws_s3_bucket.magic-helpdesk-integration.website_endpoint
    origin_id   = "S3_${aws_s3_bucket.magic-helpdesk-integration.bucket}"
  ...
  default_cache_behavior {
    target_origin_id = "S3_${aws_s3_bucket.magic-helpdesk-integration.bucket}"
  ...
  viewer_certificate {
    ...
    acm_certificate_arn      = aws_acm_certificate_validation.magic-helpdesk-integration.certificate_arn
  }
}
...
# This resource implicitly depends on the 'aws_acm_certificate_validation.magic-helpdesk-production' resource
# This resource relies on the AWS Terraform provider being previously configured
#
# AWS source: https://aws.amazon.com/cloudfront/
# Terraform source: https://www.terraform.io/docs/providers/aws/r/cloudfront_distribution.html
#
# Tags are not supported by this resource
resource "aws_cloudfront_distribution" "magic-helpdesk-production" {
  enabled = true
  comment = "MAGIC Helpdesk (Production)"
  ...
  aliases = [
    aws_s3_bucket.magic-helpdesk-production.bucket,
  ]
  ...
  # Note: CloudFront to the Origin uses HTTP, End-consumers to CloudFront uses HTTPS
  origin {
    domain_name = aws_s3_bucket.magic-helpdesk-production.website_endpoint
    origin_id   = "S3_${aws_s3_bucket.magic-helpdesk-production.bucket}"
  ...
  # Behaviours
  default_cache_behavior {
    target_origin_id = "S3_${aws_s3_bucket.magic-helpdesk-production.bucket}"
  ...
  viewer_certificate {
    ssl_support_method       = "sni-only"
    minimum_protocol_version = "TLSv1.1_2016"
    acm_certificate_arn      = aws_acm_certificate_validation.magic-helpdesk-production.certificate_arn
  }
}
